...
abstract class TweetSet {
  ...
  def union(that: TweetSet): TweetSet = {
    def unionIter(tweets : TweetSet, accu : TweetSet) : TweetSet = {
      if(tweets.isEmpty) accu
      else if(accu.isEmpty) tweets
      else unionIter(tweets.tail, accu.incl(tweets.head))
    }
    unionIter(that, this)
  }
...
object GoogleVsApple {
  ...
  val appleTweets: TweetSet = 
    TweetReader.allTweets.filter(tweet => apple.exists(str => tweet.text.contains(str)))
